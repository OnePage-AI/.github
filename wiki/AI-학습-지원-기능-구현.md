...
본 문서는 학습 자료(PDF, 문서 등) 중 사용자가 선택한 범위를 다양한 학습 형태로 다시 구성해주는 핵심 AI 기능들의 내부 파이프라인과 시스템 아키텍처를 상세히 기술합니다. 모놀리식 구조의 한계를 벗어나 역할 기반으로 분리된 백엔드 시스템과, 대용량 데이터를 안정적으로 처리하기 위한 비동기 파이프라인 설계에 초점을 맞추고 있습니다.
...
* **비용 최적화(FinOps) 및 캐싱 전략:** 변동성이 적은 분석 중간 산출물(뼈대 데이터, 팩트 추출물 등)을 고유 해시 키 기반으로 RDBMS에 캐싱하여, 중복 요청 시 불필요한 API 호출을 방지하고 레이턴시를 줄였습니다.
